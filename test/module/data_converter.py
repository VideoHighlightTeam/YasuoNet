...
def generate_segment_data(config, title, video_path, output_dataset_dir):
    # print(title, video_path)
    ...
    for i, raw_data in enumerate(iter_segment_raw_data(config, title, video_path)):
        ...
        # segment 데이터 저장
        output_path = os.path.join(output_segment_dir, f'seg_{i:05d}.pkl')
...
def convert_data(input_video_dir, segment_length, video_sample_rate, video_width, video_height, audio_sample_rate, apply_mfcc, output_dataset_dir=None):
    ...
    hl_section_path_list = glob.glob(os.path.join(input_video_dir, '*.mp4'))
    ...
    with mp.Pool() as pool:
        params = zip(repeat(config), title_list, video_path_list, repeat(output_dataset_dir))
